...
- (void)viewDidLoad
{
    ...
    [self.productcollect registerClass:[localgoodsCell class] forCellWithReuseIdentifier:@"localgoodsCell"];
...
-(void)loadproclass
{
    ...
    int i;
    NSInteger count=[proclassarray count];
    UIButton *btn;
    UIImageView *endimg;
    UIImageView *lineimg;
    for (i=0; i<count; i++) {
...
-(void)styleselect:(UIButton *)btn
{
    ...
    if (btntag==0) {
        ...
        [btn setBackgroundImage:[UIImage imageNamed:@"proclass_sed"] forState:UIControlStateNormal];
        style=[NSString stringWithFormat:@"%ld",(long)btntag];
    }
    if (btntag!=0) {
...
{
    localgoodsCell *cell = (localgoodsCell *)[collectionView dequeueReusableCellWithReuseIdentifier:@"localgoodsCell" forIndexPath:indexPath];
    ...
    if ([entity.producttype isEqualToString:@"1"]) {
        NSString *pic1=entity.Pro_smallpic;
        NSArray *fullth=[entity.Pro_bigpic componentsSeparatedByString:@","];
        if (![pic1 isEqualToString:@""]) {
            cell.productImage.image=[[UIImage alloc] initWithContentsOfFile:pic1];
